...
lets_correct_some_fucking_code = None
...
class DialogDisplay:
    ...
    def call(self, callback):
        def rollback(exitcode, exitstr):
            view.main_loop.widget = view.interface
            view.main_loop.palette = ()
            view.main_loop.unhandled_input = lambda key: None
...
class MenuItem(urwid.Text):
    ...
    def keypress(self,size,key):    
        if key == "enter":
            self.state = True
            lets_correct_some_fucking_code.button_press(0)
        return key
...
def do_menu(text, height, width, menu_height, *items):
    ...
    global lets_correct_some_fucking_code
    lets_correct_some_fucking_code = d
